...
layer_size = 314   # Define the number of nodes per layer. We used the BNA, with some regions removed
weighted = False # We are now using MST matrices. Matrices are thus not weighted - if weighted, change to True

print("")
print("")
print("########################")
print("Layer size set to ", layer_size)
print("########################")
print("")
print("")
...
# 
repsubj=df_subj['subjID'].repeat(layer_size).reset_index(drop=True)
df_repsubj=pd.DataFrame({'subjID':repsubj})

node_col=np.tile(np.arange(1,layer_size+1),len(df_repsubj) // layer_size)
network_col=np.tile(df_parcels['network'],len(df_subj))
nodename_col=np.tile(df_parcels['region'],len(df_subj))

if len(node_col) != len(repsubj):
    raise Exception("number of rows not consistent between dataframes")
#df_repsubj['nodes']=node_col
df_repsubj['node_name']=nodename_col
df_repsubj['network']=network_col
df_repsubj['network']=df_repsubj['network'].astype('category')
...
data = supra_mst
filename = outdir + '/CORE_mstEC'
colname = 'eigenvector'
layers = [0,1]
list_of_layers=[0,1]
list_of_single_layers=[0,1]

function_output(function, data, filename, colname, layers)


#compute eccentricity
function = group_eccentricity
data = supra_mst
filename = outdir + '/CORE_mstecc'
colname = 'eccentricity'
layers = [0,1]
list_of_layers=[0,1]
list_of_single_layers=[0,1]
...
#compute betweennness centrality
function = group_bet_centrality
data = supra_mst
filename = outdir + '/CORE_mstBC'
colname = 'BC'
layers = [0,1]
list_of_layers=[0,1]
list_of_single_layers=[0,1]
...
#### write output to dataframes with subjid, node and network id ######

df_ectemp=pd.read_csv(os.path.join(outdir,'CORE_mstEC.csv'),usecols=['eigenvector'])
df_ecctemp=pd.read_csv(os.path.join(outdir,'CORE_mstecc.csv'),usecols=['eccentricity'])
df_BCtemp=pd.read_csv(os.path.join(outdir,'CORE_mstBC.csv'),usecols=['BC'])

# combine
df=df_repsubj.join(df_ectemp).join(df_ecctemp).join(df_BCtemp).set_index(['subjID','node_name','network'])

# average for subnetworks and entire network
subnetworkmeans=df.groupby(['subjID','network']).mean()
networkmean=df.groupby('subjID').mean()


networkmean.to_csv(os.path.join(outdir,('CORE_ML_' + session + '_global.csv')))

subnetworkmeans.to_csv(os.path.join(outdir,('CORE_ML_' + session + '_system.csv')))
