...
for (acq in c('multi', 'dwi', 'func')) {
  ...
  outputfile <- file.path(basedir,
                          'stats',
                          paste0("CORE_MixedModel_global_acq-", acq, "_", date, ".xlsx"))
  outputfile2 <- file.path(basedir,
                           'stats',
                           paste0("CORE_LOSO_global_acq-", acq, "_", date, ".xlsx"))
  ...
  for (j in 1:length(dptvars)) {
    ...
    for (i in 1:length(graphmeasures)) {
      ...
      if (covinterest == 'dYBOCS_dCAPS') {
        # crude model
        model_graph_crude <- lm(formula = as.formula(paste(
          graph, "~ ", covinterest, " + baseline_sev"
        )), data = dfwide)
        # adj model
        model_graph_adj <- lm(formula = as.formula(
          paste(graph, "~ ", covinterest, " + baseline_sev + age + seks")
        ), data = dfwide)
  ...
  df_resp <- results_list$df_2
  df_resp_trial <- results_list_trial$df_2
  df_resp_treat <- results_list_treat$df_2
  
  if ('dYBOCS_dCAPS' %in% dptvars) {
    df_deltaybocs <- results_list$df_3
    df_deltaybocs_trial <- results_list_trial$df_3
    df_deltaybocs_treat <- results_list_treat$df_3
    
    write.list <- list(
      "delta-ybocs" = df_deltaybocs,
      "delta-ybocs-trial" = df_deltaybocs_trial,
      "delta-ybocs-treat" = df_deltaybocs_treat,
      "perchange" = df_percchange,
      "perchange_trial" = df_percchange_trial,
      "perchange_treat" = df_percchange_treat,
      "responder" = df_resp,
      "responder_trial" = df_resp_trial,
      "responder_treat" = df_resp_treat
    )
  } else {
    write.list <- list(
      "perchange" = df_percchange,
  ...
  for (LOSO in c('trial', 'treatment')) {
    ...
    if (LOSO == 'trial') {
      trials <- unique(dfwide$trial)
    } else if (LOSO == 'treatment') {
      trials <- unique(dfwide$treatment)
    }
  ...
  df_LOSO_trial_responder <- results_list$df_2_trial
  df_LOSO_trt_percchange <- results_list$df_1_treatment
  df_LOSO_trt_responder <- results_list$df_2_treatment
